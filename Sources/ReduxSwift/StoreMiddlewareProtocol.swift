...
/// Defines a protocol for middleware plugins that can be registered with a [Store](x-source-tag://ReduxStore) instance.
///
...
public protocol StoreMiddlewareProtocol: AnyObject {
    ...
    ///   - next: Function to invoke to pass-on the current action to the next plugin in the chain of middleware plugins registered with the store. This must be called just once at the end of this function to signal that the plugin is done with its operations.
    ///   - a: The `next` function has to be passed the current [action] (x-source-tag://StoreActionProtocol)
